const NIGHTLY_REGEX = /nightly/i;
const STAGING_REGEX = /staging/i;
...
async function main() {
	...
	const url =
		"https://api.github.com/repos/AztecProtocol/aztec-packages/releases?per_page=100";
	...
	if (!Array.isArray(data)) {
		console.error(
			`Unexpected GitHub API response (expected array): ${JSON.stringify(data, null, 2)}`
		);
		process.exit(1);
	}
	...
	const release = data.find(
		(r) =>
			!(
				r.draft ||
				NIGHTLY_REGEX.test(r.tag_name) ||
				STAGING_REGEX.test(r.tag_name)
			)
	);
	if (!release) {
